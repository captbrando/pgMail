...
create function pgmail_multipart(mailfrom text,mailto text,subject text,message_html text,message_plain text default '') returns int4 as '
set mailfrom $1
set mailto $2
set mailsubject $3
set mailmessage_html $4
set mailmessage_plain $5
set myHost "<ENTER YOUR MAILSERVER HERE>"
set myPort 25
set mySock [socket $myHost $myPort]
set toemailaddress_start [string first "<" $mailto]
...
if {$mailmessage_html ne ""} {
    puts $mySock "--just_a_simple_boundary"
    puts $mySock "Content-type: text/html; charset=UTF-8"
    puts $mySock "Content-Transfer-Encoding: 8bit"
    puts $mySock ""
    puts $mySock "$mailmessage_html"
}
if {$mailmessage_plain ne ""} {
    puts $mySock "--just_a_simple_boundary"
    puts $mySock "Content-type: text/plain; charset=UTF-8"
    puts $mySock "Content-Transfer-Encoding: 8bit"
    puts $mySock ""
    puts $mySock "$mailmessage_plain"
}
puts $mySock "--just_a_simple_boundary--"
puts $mySock ""
puts $mySock "."
